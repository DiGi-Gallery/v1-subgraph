...
export class AuctionHouse extends ethereum.SmartContract {
  ...
  digiAddress(): Address {
    let result = super.call("digiAddress", "digiAddress():(address)", []);

    return result[0].toAddress();
  }

  try_digiAddress(): ethereum.CallResult<Address> {
    let result = super.tryCall("digiAddress", "digiAddress():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
